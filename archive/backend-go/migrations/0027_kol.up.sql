...
CREATE TABLE creator_bookings (
    ...
    agreed_rate          DECIMAL(20,2) NOT NULL,                      -- Agreed Rate (Rp, mandatory)
    ...
    sla_target_hours     DECIMAL(10,2) NULL,                          -- Task-level SLA Target (DATA_MODEL §3); NULL => Speed Score N/A, never backfilled
    hours_logged         DECIMAL(10,2) NULL,                          -- self-reported effort (§7 Rule 2), optional, never scored
    assigned_coordinator VARCHAR(64)   NULL,                          -- owning KOL Coordinator (§3); NULL until assigned
    attributed_gmv       DECIMAL(20,2) NULL,                          -- §10.3 / M9-OA-4: written back via trackable affiliate link; NULL if none (never estimated). No writer in C1.
    created_at           DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at           DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    created_by           VARCHAR(64)   NOT NULL,
    KEY idx_bkg_brief_status (brief_id, status),
    KEY idx_bkg_coordinator (assigned_coordinator),
...
CREATE TABLE creator_payment_requests (
    ...
    amount           DECIMAL(20,2) NOT NULL,                         -- must match Agreed Rate
    ...
    updated_at       DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    created_by       VARCHAR(64)   NOT NULL,
    KEY idx_cpr_booking (booking_id),
...
CREATE TABLE creator_lists (
    ...
    last_compiled      DATETIME    NULL,                  -- when the snapshot was last produced
    created_at         DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at         DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    created_by         VARCHAR(64) NOT NULL,
