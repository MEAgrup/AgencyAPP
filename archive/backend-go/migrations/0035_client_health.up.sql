...
CREATE TABLE client_health_snapshots (
    ...
    final_health_score DECIMAL(6,3) NULL,                   -- 0..100 (Rule 7); NULL only if no component was available (defensive, renders "—")
    band               VARCHAR(16)  NOT NULL,               -- 'Healthy' | 'Watch' | 'At Risk' (Rule 7)
    roas_toggle_state  TINYINT(1)   NOT NULL,               -- effective ROAS-inclusion intent at compute time (Rule 13 / §5.1, for audit)
    components_json    JSON         NOT NULL,               -- per-component {raw, capped, base_weight, effective_weight, included, excluded_reason} (§5.1/§5.6)
    ...
    UNIQUE KEY uq_chr_client_period (client_id, period_start),
    KEY idx_chr_client (client_id),
...
FOR EACH ROW SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'client_health_snapshots is immutable: UPDATE forbidden';

CREATE TRIGGER client_health_snapshots_no_delete BEFORE DELETE ON client_health_snapshots
FOR EACH ROW SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'client_health_snapshots is immutable: DELETE forbidden';
...
ALTER TABLE clients
    ADD COLUMN roas_health_included_override TINYINT(1) NULL DEFAULT NULL
        AFTER total_sales;
