...
CREATE TABLE performance_snapshots (
    ...
    profile_score  DECIMAL(6,3) NULL,                   -- weighted KPI Profile (Rule 2), post-redistribution; NULL only if all-excluded
    modifier_value DECIMAL(6,3) NOT NULL DEFAULT 0,     -- Client-Outcome Modifier (Rule 3, clamp ±10); 0 when absent (no CHR source)
    final_score    DECIMAL(6,3) NULL,                   -- profile + modifier, bounded 0..100 (Rule 4); NULL only if all-excluded (renders "—")
    components_json JSON        NOT NULL,               -- per-component {raw, capped, base_weight, effective_weight, included, diagnostic, excluded_reason} + modifier {value, source_clients, source_component} (§5.1/§5.5)
    ...
    UNIQUE KEY uq_perf_staff_period (staff_id, period_start),
    KEY idx_perf_staff (staff_id),
    KEY idx_perf_role_period (role_type, period_start),
...
FOR EACH ROW SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'performance_snapshots is immutable: UPDATE forbidden';

CREATE TRIGGER performance_snapshots_no_delete BEFORE DELETE ON performance_snapshots
FOR EACH ROW SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'performance_snapshots is immutable: DELETE forbidden';
...
CREATE TABLE perf_kpi_weights (
    ...
    weight     DECIMAL(6,3) NOT NULL,                   -- out of 100; Σ per role_type must equal 100 (validated in code)
...
CREATE TABLE perf_period_targets (
    ...
    target_value   DECIMAL(20,4) NOT NULL,              -- normalisation denominator (units per component)
    is_placeholder TINYINT(1)    NOT NULL DEFAULT 1,    -- 1 = illustrative seed, NOT a confirmed target (O9 open)
    updated_at     DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    updated_by     VARCHAR(64)   NOT NULL,
...
INSERT INTO perf_period_targets (role_type, component, period_start, target_value, is_placeholder, updated_by) VALUES
    ('Creative', 'output_quantity',          '0001-01-01', 10,           1, 'SYSTEM'),   -- 10 approved assets / month (placeholder)
    ('Creative', 'gmv_impact',               '0001-01-01', 100000000,    1, 'SYSTEM'),   -- Rp100.000.000 attributed GMV / month (placeholder)
    ('Ads',      'gmv_impact',               '0001-01-01', 100000000,    1, 'SYSTEM'),   -- Rp100.000.000 GMV / month (placeholder)
    ('Ads',      'optimization_activity',    '0001-01-01', 20,           1, 'SYSTEM'),   -- 20 optimizations / month (placeholder)
    ('KOL',      'creator_count',            '0001-01-01', 10,           1, 'SYSTEM'),   -- 10 QC-passed creators / month (placeholder)
    ('AM',       'complaint_resolution_speed','0001-01-01', 48,          1, 'SYSTEM');   -- 48h resolution SLA (placeholder)
