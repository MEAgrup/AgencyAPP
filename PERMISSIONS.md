...
| Module | Rule |
|---|---|
| M0 Sales | Salesperson: own attempts only. **Negotiation approval: Superior (Sales Head/SPV) only.** Closing: Primary Salesperson; allocation Σ=100% enforced. **Kinerja Sales (M0 §7.1, S-01):** Sales staff = own row only (`prospect_attempts`/`clients`/`transactions`/`installments`/`sales_targets`); Sales lead/SPV = **whole division** on all five, matching the Finance/Account precedent (`jwt_is_lead() AND jwt_division() = 'Sales'`) — closed 2026-08-29, was previously staff-only-in-practice for a Head/SPV reading under RLS (`DECISIONS.md` Kinerja Sales #1). OD read-all + **writes Sales OKR** (`sales_targets`, the one write path a pure-OD account has here); Director full. |
...
| PX-M3-B Product Exchange | **Konfirmasi kategori (`px_sku_kategori`, halaman Kandidat PX): lingkup SAMA `canIsiShopId`** — `productexchange.canKonfirmasiKategoriSku` (AM pemilik klien ATAU lead Account/Director), predikat SENDIRI bukan alias (jangan menumpang). **Kandidat PX (`GET /px/kandidat`): scoping di DOMAIN, bukan gerbang binari** — AM melihat kliennya sendiri, Lead Account/Director/**OD** melihat semua (`listKandidat`, `clients.assigned_am_id`). **Katalog PX + laporan `kreator_kosong`: Lead Account/Director/OD SAJA** (`productexchange.canLihatKatalogPx`) — AM staff TIDAK melihat (beda dari Kandidat: katalog adalah bacaan agregat lintas klien, bukan alat kerja per-klien). **Bridge coverage (`POST /internal/bridge/px-coverage`) dan tick (`POST/GET /internal/px/evaluate/tick`): BUKAN aktor berlogin** — secret bersama (`BRIDGE_PX_SECRET`/`PLAN_TICK_SECRET`+`CRON_SECRET`), fail-closed bila tidak dikonfigurasi. Nav: `/px/kandidat` = AM divisi Account (staff/lead) + Director; `/px/katalog` = Lead Account/Director/OD. |
