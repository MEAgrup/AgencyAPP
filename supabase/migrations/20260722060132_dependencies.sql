-- Port dari backend/migrations/0034_dependencies.up.sql (MySQL) — konversi per docs/SUPABASE_MIGRATION_TECH_APPENDIX.md §A
--
...
CREATE TABLE dependencies (
    ...
    satisfied_notified_at timestamptz(6) NULL,                   -- fire-once EvDependencySatisfied bookkeeping (not status)
    ...
    CONSTRAINT uq_dep_pair UNIQUE (source_type, source_id, target_type, target_id)
);

CREATE INDEX idx_dep_target ON dependencies (target_id);  -- gate lookup: Blocking deps targeting a Brief
CREATE INDEX idx_dep_source ON dependencies (source_id);  -- emission lookup: deps whose Source is a Brief
CREATE INDEX idx_dep_client ON dependencies (client_id);  -- Client Board scope
