...
DECLARE
  ...
  helper_policy text[] := ARRAY[
    ...
    -- bawah memperlakukan keduanya sama: wajib terbuka untuk `authenticated`,
    -- terlarang untuk `anon`.
    --
    -- `working_days_between(date,date)` (20260907020000) — SECURITY DEFINER
    -- JUSTRU supaya pemanggil ber-RLS bisa menghitung hari kerja TANPA diberi
    -- akses ke `hari_libur` (tabel itu sengaja default-deny total: RLS nyala,
    -- nol policy, nol GRANT). Ia mengembalikan integer, tidak pernah satu baris
    -- pun kalender. Mencabutnya = `GET /briefs/{id}/stage` 500 untuk setiap
    -- pembaca divisi/AM, persis regresi yang migrasi itu perbaiki.
