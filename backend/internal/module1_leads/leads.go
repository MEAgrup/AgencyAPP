...
// RegisterInput carries the Sales single-registration fields. The json tags
// are the wire contract with web-internal lib/leads.ts (snake_case, same as
// BulkRow) — without them the FE payload decodes to zero values and every
// registration is rejected as incomplete.
