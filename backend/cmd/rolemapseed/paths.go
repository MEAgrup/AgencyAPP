...
// findSeedFile walks up from the current working directory looking for the seed
// CSV, checking two layouts at every level:
//
//	<dir>/supabase/seed/<name>  — canonical since 2026-07-29 (repo root). The
//	                              real HRIS→CDPS mapping data lives outside
//	                              backend/ so archiving the Go tree at C-05
//	                              does not archive live organisational data.
//	<dir>/seed/<name>           — legacy layout (next to go.mod), kept so an
//	                              older checkout or a hand-placed copy still
//	                              resolves.
//
// Same walk-up convention as db.FindMigrationsDir / mslseed.FindSeedCSV, so this
// works whether rolemapseed is launched from the repo root, backend/, or a
// subpackage's test cwd. envVar, if set, wins outright.
