...
const WIRE_TO_FE: Record<string, string> = {
  ...
  // Bridge MSDPS→CDPS Fase 1 — bridge.ts.
  BridgeOrderSummaryWire: 'bridge.ts::BridgeOrderSummary',
  BridgeOrderDetailWire: 'bridge.ts::BridgeOrderDetail',
  BridgeOrderPayloadWire: 'bridge.ts::BridgeOrderPayload',
  BridgeMerchantWire: 'bridge.ts::BridgeMerchant',
  BridgeAttestationWire: 'bridge.ts::BridgeAttestation',
  BridgeLineWire: 'bridge.ts::BridgeLine',
  BridgeCandidateWire: 'bridge.ts::BridgeCandidate',
  ExternalServiceMapWire: 'bridge.ts::ExternalServiceMap',
  ...
  // M19 separuh SCS. `ScsTaskWire::client_id`/`client_name` sengaja `string |
  // null` di KEDUA sisi: baris "all client" tidak punya klien, dan sebuah
  // `string` di sisi FE akan membuat halaman merender "undefined" alih-alih
  // label "Semua klien".
  ScsKategoriWire: 'scs.ts::KategoriRow',
  ScsTaskWire: 'scs.ts::ScsTaskRow',
  ScsMetricsWire: 'scs.ts::ScsMetrics',
  ScsPicSummaryWire: 'scs.ts::ScsPicSummary',
  ScsTaskBody: 'scs.ts::ScsTaskInput',
  ScsKategoriBody: 'scs.ts::KategoriInput',
  ...
  // Gelombang 4 — TikTok Ads Scanner (AS-01..AS-04). Same posture as the
  // Gelombang 3 block above: no page consumes these yet, and the FE type file
  // exists precisely so this guard watches the shape from day one rather than
  // from whenever the UI ticket lands.
  ...
  // PDT G1-09-KONFIRMASI-IDENTITAS — konfirmasi identitas satu-kali AM (Rule 2/4).
  PdtKonfirmasiIdentitasWire: 'pdt.ts::PdtKonfirmasiIdentitas',
  ...
  // PDT G2-01 lanjutan — bagian "live" (2026-09-16).
  PdtLaporanLiveWire: 'pdt.ts::PdtLaporanLive',
  // PDT G2-01 lanjutan — bagian "video" (2026-09-16, TikTok-only).
  PdtLaporanVideoWire: 'pdt.ts::PdtLaporanVideo',
