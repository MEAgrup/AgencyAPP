/**
 * The `/internal/px/evaluate/tick` secret gate (Product Exchange M3-B).
 * These cases all reject BEFORE the handler touches the database, so they
 * need no DATABASE_URL — pola sama `pdt/purge/tick/route.test.ts`, secret
 * SAMA `PLAN_TICK_SECRET`/`CRON_SECRET` yang tick lain pakai.
 *
 * AKTIVASI DITUNDA (lihat docblock route.ts): tidak ada cron yang memanggil
 * route ini hari ini, tapi gerbangnya tetap harus fail-closed sejak hari
 * pertama — sama seperti seluruh tick lain sebelum diaktifkan.
 */
import { afterEach, beforeEach, describe, expect, it } from 'vitest';
import { GET, POST } from './route';

const URL = 'http://localhost/api/v1/internal/px/evaluate/tick';
...
describe('POST/GET /internal/px/evaluate/tick — secret gate', () => {
