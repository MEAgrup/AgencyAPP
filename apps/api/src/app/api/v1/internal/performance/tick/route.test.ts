/**
 * The `/internal/performance/tick` secret gate (M14-G1). These cases all reject
 * BEFORE the handler touches the database, so they need no DATABASE_URL — they
 * prove the one thing that must never regress: an unset or wrong secret cannot
 * drive the monthly snapshot sweep. The secret is the SAME `PLAN_TICK_SECRET` the
 * plan tick uses (reused, not a second credential).
 */
import { afterEach, beforeEach, describe, expect, it } from 'vitest';
import { POST } from './route';

const URL = 'http://localhost/api/v1/internal/performance/tick';
...
describe('POST /internal/performance/tick — secret gate', () => {
