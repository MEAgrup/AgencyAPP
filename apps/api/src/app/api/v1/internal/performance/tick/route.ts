 ...
 * and the penugasan tick use, deliberately reused rather than minting a second system
 * credential to rotate. An unset secret means the endpoint is CLOSED, never open: a
 * missing env var must not turn a privileged system hook into an anonymous one. The
 * concrete cron provider is Vercel Cron (owner decision 2026-08-19), wired in
 * `vercel.json`.
 *
 ...
 * `performance.runSnapshotJob` writes one immutable per-staff snapshot for the
 * most-recently CLOSED WIB calendar month. It is idempotent — a re-run finds the
 * snapshots already present — so a double call is a no-op the second time. `waktu` in
 * the POST body (RFC3339) overrides "now" for backfill/testing; the default is the
 * wall clock. Because the closed month is derived in WIB, the monthly cron must fire
