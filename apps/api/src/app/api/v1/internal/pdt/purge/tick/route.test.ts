/**
 * The `/internal/pdt/purge/tick` secret gate (PDT G1-10). These cases all
 * reject BEFORE the handler touches the database, so they need no
 * DATABASE_URL — they prove the one thing that must never regress: an unset
 * or wrong secret cannot drive the daily purge sweep. The secret is the SAME
 * `PLAN_TICK_SECRET`/`CRON_SECRET` every other tick uses (reused, not a
 * second credential).
 */
import { afterEach, beforeEach, describe, expect, it } from 'vitest';
import { GET, POST } from './route';

const URL = 'http://localhost/api/v1/internal/pdt/purge/tick';
...
describe('POST /internal/pdt/purge/tick — secret gate', () => {
