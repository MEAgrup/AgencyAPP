/**
 * The `/internal/pdt/reparse/tick` secret gate (PDT G1-11). These cases all
 * reject BEFORE the handler touches the database, so they need no
 * DATABASE_URL — pola sama `pdt/purge/tick/route.test.ts` — the secret is
 * the SAME `PLAN_TICK_SECRET`/`CRON_SECRET` every other tick uses.
 */
import { afterEach, beforeEach, describe, expect, it } from 'vitest';
import { GET, POST } from './route';

const URL = 'http://localhost/api/v1/internal/pdt/reparse/tick';
...
describe('POST /internal/pdt/reparse/tick — secret gate', () => {
