/**
 * PUT /api/v1/strategi/{id}/risks — Section H-1, the risk register.
 *
...
import { strategiDetailToWire, strategiRisksFromWire } from '@/lib/wire';
...
export async function PUT(request: Request, ctx: { params: Promise<{ id: string }> }): Promise<Response> {
  return handle(async () => {
    ...
    const saved = await strategi.saveRisks(db(), actor, id, strategiRisksFromWire(b.risks ?? []));
