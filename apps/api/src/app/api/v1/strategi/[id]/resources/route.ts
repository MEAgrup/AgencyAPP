/**
 * PUT /api/v1/strategi/{id}/resources — Section F, the soft resource commitment (Rule 10).
 *
...
import { strategiDetailToWire, strategiResourcesFromWire } from '@/lib/wire';
...
export async function PUT(request: Request, ctx: { params: Promise<{ id: string }> }): Promise<Response> {
  return handle(async () => {
    ...
    const saved = await strategi.saveResources(db(), actor, id, strategiResourcesFromWire(b.resources ?? []));
