 ...
 * Gelombang C (C-03). Cakupan bisnis (revisi baru = salinan revisi 0)
 * dibuktikan `packages/domain/src/pdt.test.ts`; di sini cukup bukti route
 * SUNGGUHAN menyambungkan auth, parsing `id`, dan pemetaan status/wire.
 */
...
const SECRET = 'test-jwt-secret-pdt-insight-reset';
...
function reqPost(token: string, id: string | number): Request {
  return new Request(`http://localhost/api/v1/account/pdt/laporan/kiriman/${id}/insight/reset`, {
    method: 'POST',
...
const owner = sign({ employeeId: 'ZZ-M20CB-AM', division: 'Account', level: 'staff', od: false, director: false });
const otherAm = sign({ employeeId: 'ZZ-M20CB-LAIN', division: 'Account', level: 'staff', od: false, director: false });
...
describe('POST /pdt/laporan/kiriman/{id}/insight/reset — gagal sebelum DB tersentuh', () => {
...
// ---------------------------------------------------------------------------
// Integrasi Postgres nyata (di-skip tanpa DATABASE_URL) — prefix 'ZZ-M20CB-'/
// 'CLI-M20CB-'.
// ---------------------------------------------------------------------------
...
let seq = 0;
const nextClientId = (): string => `CLI-M20CB-${Date.now() % 100000}-${seq++}`;
...
async function fixtureKirimanId(): Promise<number> {
  ...
  await insertClient(clientId, 'ZZ-M20CB-AM');
...
beforeAll(async () => {
  ...
  await sql`
    ...
    values ('ZZ-M20CB-AM', 'AM Uji Insight Kiriman', 'zz-m20cb-am@mea.co.id', 'Account', 'Account Manager', true, 'SYSTEM')
    on conflict (employee_id) do nothing`;
});
afterAll(async () => {
  if (!sql) return;
  await sql`delete from employees where employee_id = 'ZZ-M20CB-AM'`;
  await sql.end();
});
afterEach(async () => {
  if (!sql) return;
  await sql`delete from pdt_laporan_publikasi where kiriman_id in (
    select id from pdt_laporan_kiriman where client_platform_id in (select id from client_platforms where client_id like 'CLI-M20CB-%'))`;
  ...
  // (insight, terbitkan, terbitkan-ulang, cabut) yang menjalankan dansa yang sama
  ...
  await sql`delete from pdt_laporan_kiriman where client_platform_id in (select id from client_platforms where client_id like 'CLI-M20CB-%')`;
  await sql`delete from client_platforms where client_id like 'CLI-M20CB-%'`;
  await sql`delete from clients where id like 'CLI-M20CB-%'`;
});

describeDb('POST /pdt/laporan/kiriman/{id}/insight/reset — real DB', () => {
  it('setelah suntingan AM ⇒ 200, revisi baru = salinan revisi 0 (bukan suntingan)', async () => {
    ...
    const body = await res.json();
    expect(body.revisi).toBe(2);
    expect(body.sumber).toBe('am');
    expect(body.ringkasan).not.toBe(DRAFT.ringkasan);
  });

  it('403 untuk AM yang bukan pemilik klien', async () => {
    const kirimanId = await fixtureKirimanId();
    const res = await POST(reqPost(otherAm, kirimanId), ctx(kirimanId));
