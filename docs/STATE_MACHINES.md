...
## 6. Service (M6)
`[Awaiting Onboarding]` → `[Strategy Approved]` (plan-gated only; Direct services skip) → `[Briefed]` (first Brief created) → `[In Execution]` (any Brief leaves `[To Do]`) → `[Completion Requested]` → `Done` (O75, dua langkah — lihat di bawah). Void Service (M4-OA-5): SPV/Account Lead approval; cascades child Briefs not yet `[Approved]` → `[Cancelled — Service Voided]`.
- **Tutup Service DUA-LANGKAH (O75, keputusan pemilik 2026-09-08):** AM pemilik **mengajukan** `[In Execution] → [Completion Requested]` (`require_lead=false`); Head of Account **menyetujui** `[Completion Requested] → Done` atau **menolak** `[Completion Requested] → [In Execution]` (keduanya `require_lead=true`). Edge langsung `[In Execution] → Done` (ada sejak `20260723055732` dengan **nol pemanggil**) **DICABUT** — pola yang sama dengan T-2b mencabut hold satu-langkah. Gate SIAPA di domain (`client.ts::requestServiceCompletion`/`approveServiceCompletion`/`rejectServiceCompletion`): request = AM pemilik / Account lead / Director; approve/reject = Head of Account (Account Lead) / Director. Alasan **wajib** saat mengajukan.
  - **Gerbang wajib: `contracts.tanggal_akhir` sudah LEWAT** — ditegakkan DUA kali, di domain dan sebagai trigger DB `trg_service_tutup_butuh_kontrak_berakhir`, keduanya menjawab `[service belum boleh ditutup sebelum kontraknya berakhir]`. Kontrak yang berakhir **hari ini** belum lewat. Tersedia karena A-4 (K-2): sejak itu setiap closing mencetak `contracts` ber-`tanggal_akhir` dan setiap `SVC-` menggantung padanya. Service **tanpa** kontrak (`contract_id` null ⇒ seluruh barisnya sekali-jadi) **lolos gerbang tanggal** — syaratnya tidak ada, bukan belum terpenuhi; persetujuan Head tetap wajib (asumsi eksplisit, `DECISIONS.md` O75).
  - **Tanggal selesai TIDAK disimpan sebagai kolom**: ia dibaca dari baris `audit_log` transisinya lewat `private.service_tanggal_selesai` (`client.completionDate`), dan **inilah sumber `accrual.AccrualInput.tanggalSelesai`** untuk layanan ber-`pengakuan = 'saat_selesai'` (38 layanan aktif di katalog). Kolom kedua yang memuat fakta yang sama bisa berbeda dari log-nya, dan log-nya yang menolak UPDATE (aturan rumah #3/#4).
  - `[Completion Requested]` **non-terminal** dan masih dianggap **aktif**: rekap mingguan (`wrr_monday_job`, D-06) dan Client Health tetap menghitungnya sampai Head menyetujui. Ia otomatis **tidak briefable** karena `account.isServiceBriefable` adalah daftar-putih. **Tidak** ada edge void dari state ini — konsisten dengan `[Hold Requested]`/`[On Hold]` yang juga tidak punya; sebuah pengajuan harus ditolak dulu sebelum Service-nya bisa di-void. Notif katalog **v16** (request→Head of Account; approve/reject→AM pemilik). Migrasi `20260925010000_o75_service_tutup.sql`.
  - **Backfill Service lama SENGAJA belum dijalankan** — menunggu ketokan "pendapatan Service yang kontraknya sudah lewat diakui di bulan MANA". Saat diketok: lewat `sm_transition` aktor `SISTEM`, dua langkah berurutan, **bukan `UPDATE` mentah**.
...
- **Anchor target GMV di-refresh di sini (O76, keputusan pemilik 2026-09-08).** `renewal_requests.target_gmv_baru` diusulkan bersama proposal (form renewal mengisinya dengan angka LAMA sebagai default, jadi "tidak berubah" adalah pilihan sadar) dan baru dipindahkan ke `clients.target_gmv` oleh `executeRenewal`, di transaksi yang sama dengan `CTR-`/`SVC-`/`TRX-`-nya, dengan audit before→after pada entitas `client` ber-`sumber: 'renewal'`. **Kenapa di eksekusi, bukan di proposal:** usulan yang belum disetujui tidak boleh menggeser anchor yang sedang mengukur floor GMV Strategi yang berjalan (O76). `NULL` = tidak diubah, berbeda dari nol. Migrasi `20260927010000_o76b_renewal_target_gmv.sql`.
