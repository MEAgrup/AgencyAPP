...
## 25. External Order `ORD-` (Bridge MSDPS→CDPS Fase 1) — mesin #35 (`sm_machines` 34→35)
`[Masuk]` → `[Diterima]` | `[Ditolak]` (keduanya terminal), plus `[Masuk]` → `[Dibatalkan Sumber]` (registered, **UNREACHABLE di Fase 1** — milik callback Fase 2). Migrasi `20261007010000_bridge_msdps_fase1.sql`. Bridge **BUKAN modul PRD** — otoritasnya `docs/DECISIONS.md` 2026-09-10 (menggantikan entri 2026-07-12 butir 7), preseden `TSK-` Penugasan Internal ("di luar 18 PRD").

| From | To | Who | Effect |
|---|---|---|---|
| `[Masuk]` | `[Diterima]` | Lead Account atau Director (`require_lead=true`, `require_division='Account'`) | `bridge.accept` menjalankan seluruh transaksi (client/service/attestation/release) SEBELUM transisi ini — transisi adalah langkah TERAKHIR |
| `[Masuk]` | `[Ditolak]` | Lead Account atau Director | `ditolak_alasan` wajib (CHECK `ck_extorders_ditolak`) |
| `[Masuk]` | `[Dibatalkan Sumber]` | (belum ada gerbang — nol `require_lead`) | Edge terdaftar untuk Fase 2 (callback pembatalan dari MSDPS); TIDAK ada route yang memicunya di Fase 1 |

- **Sumber**: MSDPS (`MEAgrup/MEAGO_MSDPS`, proyek Supabase terpisah). MEAGO! menutup deal dengan merchant POI tapi tidak punya staff Account/Ads/Creative/Store Operation sendiri — semuanya bekerja di CDPS. `external_orders` adalah inbox satu arah: MSDPS mendorong (`POST /api/v1/internal/bridge/orders`, secret `BRIDGE_INGEST_SECRET` terpisah dari `CRON_SECRET`), manusia CDPS menerima atau menolak. **Nol callback ke MSDPS di Fase 1.**
- **`payload` IMMUTABLE via trigger** (`external_orders_payload_frozen`, blok UPDATE) — pola `client_reports.payload`. Bentuknya `docs/BRIDGE_MSDPS_CONTRACT.md` v1; versi tak dikenal ditolak `bridge.intake` dengan pesan BI eksplisit, bukan CHECK constraint (supaya v2 kelak adalah kode baru, bukan migrasi yang mencabut constraint).
- **Idempoten by construction**: `idempotency_key` (`<DEAL code MSDPS>:<payload_versi>`) UNIQUE. `bridge.intake` mengembalikan `ord_code` asli pada key yang berulang — nol baris baru, nol notifikasi kedua. Ini yang membuat retry outbox MSDPS (setelah timeout ambigu) aman.
- **`bridge.accept` adalah SATU transaksi** yang, sebelum menyentuh `sm_transition`, sudah: (1) memvalidasi atestasi pembayaran ADA di payload (`diverifikasi_pada` — menolak untuk actor mana pun termasuk Director, ini jaminan sisi CDPS di balik D4/D13); (2) meresolusi setiap baris bridge ke Master Service List lewat `external_service_map` — **baris tak terpetakan ⇒ SELURUH accept gagal** dengan `[layanan MEAGO belum dipetakan ke Master Service List]`, atomik, nol `CLI-`/`SVC-`/`client_external_ref` tertinggal; (3) attach ke `CLI-` yang sudah ada (pilihan manusia dari `bridge.candidates`, TIDAK PERNAH auto-select) atau membuat `CLI-` baru ber-`sumber='meago'`; (4) insert `client_external_ref` (UNIQUE menangkap race dua accept bersamaan pada `external_id` yang sama — insert langsung, nol cek-dulu-insert); (5) insert `client_external_billing` (atestasi, append-only — lihat berkas migrasi untuk trigger-nya); (6) stempel `clients.released_to_account_at` (gerbang intake Account M5 §5 — bridge TIDAK menugaskan AM, klien masuk Unassigned Intake Queue seperti biasa); (7) BARU transisi `[Diterima]`.
- **Nol `TRX-` CDPS untuk order MEAGO.** `clients.transaction_id`/`payment_intent` tetap NULL — transaksi bayangan akan masuk antrean verifikasi M5, dashboard reminder, accrual, dan Kinerja Sales (lihat `clients.sumber`/`services.sumber` di `docs/DATA_MODEL.md` §1 dan A7 anti-drift di `docs/DECISIONS.md` 2026-09-10).
- **D5 (opsi b, amandemen)**: `sales_pic_id`/`commission_payment_pic_id` klien baru = employee layanan tunggal "MEAGO Bridge" (prasyarat non-kode, nol baris `role_mappings`). Identitas BD MEAGO tetap terekam di `payload` (imutabel) + `client_external_billing.diverifikasi_oleh_external`.
- **D10 (dibaca ulang)**: gerbang tulis = `permission.isLead(actor, 'Account')`, sama persis `sm_edges.require_lead + require_division='Account'` — BUKAN "Head Account saja" (tier itu belum ada di model peran, wave K-1 sengaja ditunda).
- Prefix `ORD-YYYYMM-NNNN` (registry baru — `entity_prefix` 43→44). **Satu event katalog baru** (v16, `bridge.order.masuk` → lead Account — `notif_events` 73→74): `[Ditolak]` nol audiens CDPS Fase 1 (nol callback), `[Diterima]` adalah aksi actor sendiri, jadi hanya `[Masuk]` yang butuh pemberitahuan.
