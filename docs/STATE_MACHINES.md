...
## 22. Periode Buku `periode_buku` (`PBK-`, Gelombang D D-3) — mesin #32 (`sm_machines` 31→32)
`Terbuka` → `Ditutup`. **SATU edge, dan `Ditutup` terminal — nol jalan buka kembali, sengaja.** Migrasi `20260923010000_d3_tutup_buku.sql`.

| From | To | Who | Effect |
|---|---|---|---|
| `Terbuka` | `Ditutup` | Head of Finance (Finance level lead) atau Director (`require_lead=true`) | Angka pengakuan bulan itu **DIBEKUKAN** ke `periode_buku_baris` (satu baris per layanan, append-only), stempel `ditutup_pada`/`ditutup_oleh` dipasang, dan sejak titik ini `tutupbuku.laporanBulan` membaca bulan itu dari angka beku — **tidak pernah** menghitung ulang |

- **Kenapa nol jalan buka kembali.** Ketokan D-3 (pemilik 2026-09-07) menuntut bulan tertutup *"tidak bisa diedit sama sekali"*, dan koreksi *"hanya lewat jurnal koreksi di bulan berjalan"*. Sebuah edge `Ditutup → Terbuka` — bahkan yang dipagari peran — adalah persis mekanisme yang membuat tuntutan itu berhenti berarti apa pun: begitu ia ada, ia akan dipakai, dan tidak ada lagi yang bisa tahu apakah angka bulan lampau pernah berubah. Bandingkan §15 (`weekly_result_recap`), yang PUNYA `Ditutup Otomatis → Terbuka` karena rekap mingguan bisa dibuka ulang oleh Head of Account, dan M6B `plan` yang tutupnya sengaja **tidak** mengunci sama sekali (X-07, koreksi pasca-tutup diterima sebagai amandemen). Buku keuangan bukan periode Plan; perbedaan itu diketok pemilik, bukan disamakan demi keseragaman.
- **Mesin statusnya bukan satu-satunya penjaga.** `sm_transition` hanya menjaga jalur yang lewat dirinya; `psql`, service-role, dan skrip masa depan tidak lewat sana. Karena itu trigger `guard_periode_buku_tertutup` menolak **setiap** `UPDATE` **dan** `DELETE` atas baris yang sudah `Ditutup`, dengan pesan BI yang menyebut OBATNYA (jurnal koreksi di bulan berjalan), bukan hanya penolakannya. `DELETE` ikut dijaga karena menghapus baris periodenya adalah cara termurah membatalkan penutupan.
- **Berkunci `bulan`, bukan `id`.** `sm_transition` dipanggil dengan `idColumn: 'bulan'` (`varchar(7)`, UNIQUE) — kolom itulah identitas bulan buku, dan sebuah surrogate sebagai kunci transisi akan mengizinkan dua baris untuk satu bulan, yang persis dilarang. `id` `PBK-YYYYMM-NNNN` tetap ada (aturan rumah #1) dan dicetak dengan stempel hari PERTAMA bulan yang diwakilinya, bukan `now()`: buku Maret tetap `PBK-202603-0001` walau ditutup di April.
- **Tiga gerbang keadaan di lapisan domain** (`tutupbuku.tutupBulan`), masing-masing dengan pesan BI-nya sendiri: bulan yang belum berakhir tidak bisa ditutup (angkanya masih bertambah setiap hari); bulan yang sudah tertutup tidak bisa ditutup dua kali; dan buku ditutup **berurutan** — masih ada bulan lebih awal yang `Terbuka` ⇒ ditolak, karena deret angka beku yang berlubang membuat sebuah jurnal koreksi bisa menunjuk bulan yang belum pernah dibekukan.
- **Nol event katalog baru** (`notif_events` tetap 69): tidak ada entri katalog Fase 0 v2 §9 yang meminta notifikasi tutup buku, dan mengarang satu berarti mengarang entri katalog. **+2 prefix** (`PBK`, `JRK`) ⇒ `entity_prefix` 40→42.
- `jurnal_koreksi` (`JRK-`) **bukan** mesin status: ia append-only dan tidak punya siklus hidup — membatalkan sebuah koreksi berarti mencatat koreksi kedua yang berlawanan, bukan mengubah statusnya.
