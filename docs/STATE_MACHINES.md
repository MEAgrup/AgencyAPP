...
- **Hold Service (T-2 / RM-2, keputusan pemilik 2026-08-14):** `[In Execution]` ↔ `[On Hold]` (dua edge, `require_lead=true`). Gate SIAPA di domain (`client.ts::holdService`/`resumeService`) = Head of Account (Account Lead / Director), mirror gate Void Service; alasan **wajib** saat hold. `[On Hold]` **non-terminal**. **Tidak cascade** ke Brief/Asset/Campaign anak — hold hanya menyetop kewajiban rekap + skoring, tak memaksa transisi anak. Konsekuensi RM-2: klien yang **semua** service-nya `[On Hold]` tak dibuka rekap mingguan (`wrr_monday_job`, D-06) — TAPI **tetap muncul di Client Health report** dengan keterangan status hold (bukan di-skip; `health.portfolio.onHold`). Migrasi `20260814030000_t2_service_hold.sql`.
