...
- Prefix `TSK-YYYYMM-NNNN` (registry). RLS `internal_tasks_select` + `GRANT SELECT TO authenticated` (tanpa GRANT, `readAsActor` ditolak sebelum policy sempat dievaluasi — `rls_checks` §43). Migrasi `20260814110000_penugasan_internal.sql`.
