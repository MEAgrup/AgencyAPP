...
## 20. Contract Renewal/Cross-Sell `RNW-` (Kinerja Sales R-03) — mesin #.., dari Client Record (nol Lead/Prospect)

`Draft` → `Negotiation - Pending Approval` | `Negotiation - Auto Approved` → `Closed`; `Negotiation - Pending Approval` → `Negotiation - Approved` | `Negotiation - Revision Required` | `Negotiation - Rejected`; `Negotiation - Revision Required` → `Negotiation - Approved` | `Negotiation - Pending Approval`; `Negotiation - Rejected` → `Negotiation - Pending Approval`. Batal: `Draft` | `Negotiation - Pending Approval` | `Negotiation - Revision Required` | `Negotiation - Rejected` → `Cancelled`. Terminal: `Closed`, `Cancelled`.

- **Kenapa mesin baru, bukan menumpang `prospect_attempt` (§1).** Renewal tidak pernah punya Lead/Prospect — klien sudah ada, tidak ada "hilang" untuk dipetakan ke Closed-Lost, dan menumpangkan alur itu berarti menulis ulang mesin closing yang sudah lama stabil & diuji hanya untuk membedakan satu kasus. `contract_renewal` mereplikasi HANYA sub-alur negosiasi (Qualified→Negotiation→Closed) dengan label status IDENTIK persis ke `prospect_attempt` (state-nya berarti sama, tidak ada kosakata kedua), ditambah satu pintu keluar yang `prospect_attempt` tidak perlukan: batal langsung dari `Draft`/Negotiation manapun ke `Cancelled` (di `prospect_attempt`, "batal" selalu lewat lead yang hilang).
- **`negotiation_proposals`/`negotiation_proposal_lines` DIPERLUAS, bukan diduplikasi.** `attempt_id` jadi nullable, kolom baru `renewal_id` (FK `contract_renewals`), CHECK memaksa tepat satu penambat terisi (`ck_neg_anchor_shape`). Penomoran versi/aturan "harga standar auto-approve, harga custom butuh ACC atasan" (`packages/domain/src/sales.ts`'s `resolveProposalLine`/`validateParties`/`validateShape`/`validateScheduleTotal`, kini di-export) dipakai ulang APA ADANYA lewat `packages/domain/src/renewal.ts` — bukan re-derive.
- **Closing renewal TIDAK mencetak `CLI-` baru** (deviasi PRD M0 §6, `DECISIONS.md` 2026-08-30 arah a) — ia melahirkan `CTR-`/`SVC-`/`TRX-` baru pada `client_id` yang SAMA. Karena itu alokasi (`client_sales_allocations`) dan `services` di-scope per **`transaction_id`** (bukan lagi `client_id` saja) supaya closing baru tidak mencampur kredit/komisi dengan closing lama pada klien yang sama — lihat `DECISIONS.md` 2026-08-30 "SP-2 RESOLVED".
- **Kredit alokasi** ikut sales yang MEMPROSES renewal (bisa berbeda dari `sales_pic_id` klien); **aturan komisi** sama dengan penjualan baru (`finance.commissionAchievement`, tidak berubah rumus, hanya di-scope ulang per transaksi).
- **Strategi kontrak baru TETAP manual oleh AM** — R-03 sengaja TIDAK memicu pembuatan `STRG-` otomatis saat renewal closing (owner decision).
- **Cross-sell SELALU kontrak baru terpisah** — tidak pernah menempel/memperpanjang kontrak yang sedang aktif, beda dari `perpanjangan` yang wajib merantai `contract_sebelumnya_id`.
- Prefix `RNW-YYYYMM-NNNN` (registry). RLS `contract_renewals_select`: `jwt_can_read_all()` OR pembuat OR `jwt_owns_client` OR lead Sales/Account sedivisi. Tulis selalu lewat service-role + gate TS (`renewal.canManageRenewal`) + `sm_transition` untuk kolom status — tanpa policy tulis (pola `contracts`).
- Migrasi: `20260901080000_r03_transaction_scoped_allocations.sql`, `20260901090000_r03_contract_renewals.sql`, `20260901100000_r03_services_transaction_id.sql`. Modul: `packages/domain/src/renewal.ts` (+ `renewal.test.ts`).
- **Status R-03/R-04 (2026-08-30): SELESAI PENUH & diuji** — domain layer, route API, wire mapper, FE lib, dan tombol/form UI `/clients/[id]` (`RenewalPanel.tsx`) semuanya mendarat. Lihat `docs/backlog/SALESPERF_BACKLOG.md` §Stream B.
