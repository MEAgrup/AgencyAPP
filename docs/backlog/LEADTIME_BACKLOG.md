...
| # | Isi | Status |
|---|---|---|
| LT-1 | Bobot `kecepatan_review_am` di profil AM | ✅ **SELESAI 2026-09-08** — bobot **10%** di-carve proporsional (45/22,5/22,5/10 → 40,5/20,25/20,25/9 + 10), Σ=100; target normalisasi **24 jam DIKONFIRMASI** (pemilik: *"buat 1 hari kerja"* — angkanya (24) tidak berubah, seed asalnya sendiri sudah menyamakan "1 hari kerja" dengan placeholder 24 jam, jadi menjawab "1 hari kerja" = mengonfirmasi angka yang sudah ada, bukan menghitung ulang). Migrasi `20260901010000_lt1_am_review_weight.sql` (bobot) + `20260925060000_lt1_confirm_review_target.sql` (`is_placeholder` true→false, satu UPDATE data) — **kedua migrasi sudah di live `CDPS SG`** (yang kedua di-apply 2026-09-08 sesudah PR #326, diverifikasi lewat kueri). Test `performance.test.ts` "LT-1: proportional carve" / "LT-1: kecepatan_review_am carries its 10%" + integrasi `LT-32: kecepatan_review_am wiring` (diperbarui: `targetsPlaceholder` sekarang `false` untuk skenario itu, bukan `true`). Bobot `role_type` AI Optimizer + Store Operation **tetap Σ=0** — **ditanyakan ulang 2026-09-08** sesudah M18 mendarat, **pemilik memilih menahan di 0** (alasan sama dengan Sales/AI Optimizer: bobot M14 §9 ditandatangani 2026-08-13, jangan dipotong ulang tanpa data nyata). Jangan tanyakan ulang bagian mana pun dari LT-1 lagi — keduanya sudah diputuskan |
...
| LT-2, LT-8 | Daftar Store Operation + alasan pengembalian brief-nya | ⬜ **MASIH MENUNGGU PEMILIK** — "menyusul". LT-8 sengaja ditahan agar dijawab bersama LT-2. **Catatan 2026-09-08 (M18):** modul Store Operation sudah dibangun TANPA menunggu ini — entitas baris SKU (`SKU-`), mesin #32, rollup, halaman, dan kuota satuan semuanya tidak menuntut `stage_pipeline` (M16 Rule 12). Yang masih kosong persis satu hal: daftar & urutan tahapannya. Biaya memasangnya nanti tetap sebagaimana dijanjikan — satu migrasi (mesin `stage_store_ops` ⇒ `sm_machines` 32→33 + gate di `db-rebuild.sh` dan `ci.yml`, satu baris `stage_pipeline`, n baris `stage_definition` dengan `Cek Brief AM` di depan + edge balik LT-4) plus kode alasan LT-8 di `REASON_CODES_BY_DIVISION`. Rinciannya `docs/prd/CDPS_Module18_Store_Ops.md` §7. **Ditanyakan ulang ke pemilik 2026-09-08** dengan opsi konkret (kirim daftarnya sekarang / pasang pipeline minimal ber-`Cek Brief AM` saja / biarkan terbuka) — **pemilik memilih biarkan terbuka**. Jadi ini bukan lagi "belum sempat dijawab" melainkan "diputuskan menunggu"; jangan tanyakan ulang |
