...
| SH-08 | **GMV kotor vs bersih dipisah (SHP-1)** — `parseBisnisHome` membaca bagian ke-3 *Pesanan Dibayar*; payload `kpi.dibayar` + `periode.gmv_bersih_sumber`; renderer dua kartu + selisih; `createReportShopee` mengisi `gmv_kotor`=Dibuat, `gmv_net`=Dibayar, sehingga `clients.total_sales` → Health Score M13 berbasis uang yang masuk. Fallback dinyatakan bukan senyap | ✅ 2026-09-03 |
| SH-09 | **Pengenalan nama berkas MENTAH Seller Centre (SHP-3)** — `detectModuleFromRawName`, lapisan nama KEDUA setelah konvensi tim dan sebelum fallback isi. Export asli: deteksi 8/15 → **15/15**, salah slot 3 → **0** | ✅ 2026-09-03 |
...
| # | Tiket |
|---|---|
| AS-01 ✅ | **Diputus O67: jalur (b) PORT PENUH** — angkanya menggerakkan keputusan sistem (skor/bucket/realokasi budget), jadi aturan pemisahnya memaksa port, bukan embed. Entri `DECISIONS.md` ada (O67, dan baris Gelombang 4 2026-09-03). Teks tiket aslinya: Pilih jalur **sadar**: (a) embed `.html` via `web-internal/public/tools/` + entri `embedded-tools.ts` + baris nav ber-gate, atau (b) port ke `packages/core/`. Aturan pemisahnya: **angka cuma dibaca manusia → embed; angka menggerakkan keputusan sistem → port** (skor yang tak bisa dihitung ulang server-side melanggar aturan rumah #4). Wajib entri `DECISIONS.md` |
