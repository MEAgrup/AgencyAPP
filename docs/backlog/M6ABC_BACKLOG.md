...
- ⚠️ **A-08 hijau di repo, BELUM di live** — live memuat migrasi Section D milik
  sesi hilang dengan bentuk D-5 yang berbeda. Lihat **O59** di `docs/DECISIONS.md`;
  jangan mengklaim repo ≡ live sampai itu diputus.
...
| X-13 | Daftar enum D-6 (leading indicator mingguan) tidak pernah ditulis PRD | 🟡 **BARU 2026-08-08.** §4 hanya bilang "Multi-enum (maks 5)". Dipakai: kosakata metrik **D-4** yang PRD tulis sendiri (10 nilai = `ck_strtg_metric`), supaya nol vocabulary dikarang; ditegakkan `<@` di DB + di-assert identik dengan `TARGET_METRICS` oleh test. Perlu konfirmasi apakah ada indikator mingguan yang BUKAN metrik D-4. Membalikkan = satu `ALTER` + satu literal. Detail di `DECISIONS.md` |
