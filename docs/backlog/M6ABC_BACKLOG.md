...
| 🔴 **A-13c** | **D-2 matriks target + D-8 asumsi + E-12 ketergantungan** | **Prasyarat semua sisa M6A.** `checkCompleteness` menuntut ketiganya dan tidak satu pun punya editor: `saveStrategiTargets`, `saveStrategiAssumptions`, `saveStrategiKetergantungan` punya **nol pemanggil** di seluruh `web-internal/src` — buktikan sendiri dengan `grep -rn` sebelum mulai. Akibatnya **tidak ada Strategi yang bisa lolos Ajukan dari UI**; setiap rekaman menampilkan hitungan kekurangan permanen. `route-parity` tidak bisa menangkapnya — ia memeriksa path yang FE panggil, bukan path yang tidak dipanggil siapa pun (kelas SESI9 §8, satu lapis lebih dalam). D-3 derivatif (X-11) — render angka, jangan input. Stretch `>=` floor sudah ditegakkan DB, jangan tulis ulang di TS. Rule 8 diperiksa terhadap target GMV, jadi UI asumsi harus menunjukkan target mana yang masih telanjang. **Selesai = `submitStrategi` menjawab 200 atas fixture Alpha Digital**, bukan "tiga form terpasang" |
