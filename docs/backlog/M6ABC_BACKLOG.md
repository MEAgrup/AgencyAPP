...
| ~~A-11~~ | ~~Tautan klien read-only `/s/{token}`~~ | ✅ **SELESAI 2026-08-09** — migrasi `20260809010000` (2 tabel, gerbang 82→84), domain `createShareToken`/`revokeShareToken`/`getShareLinkStatus`/`resolveShareLink`, route internal `POST/GET/DELETE /strategi/{id}/share-link`, halaman publik `apps/api/app/s/[token]`, kontrol `ShareLinkPanel`. Token ter-hash SHA-256 diikat ke `contract_id` (resolusi selalu ke versi `Aktif`; Draft tak terjangkau), satu Aktif/kontrak, revocable + expirable (default akhir kontrak + 30 hari), access-logged append-only. Filter §7 lewat pintu tunggal `shareableFieldIds()` SEBELUM serialisasi; `clientView()` aman by-construction. **X-06/RA-7 diputus pemilik: versi aktif saja, tanpa riwayat/diff.** 11 test domain. Poles layout HTML klien ditunda (kontrak keamanan lengkap & teruji). Lihat DECISIONS 2026-08-09 |
