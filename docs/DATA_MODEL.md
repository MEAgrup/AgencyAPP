...
| Target Metrik Brief Ads | — (PK `brief_id`) | BRF 1:1 | M8 | **Keputusan pemilik 2026-08-14** (`DECISIONS.md`). `ads_brief_targets` — enam angka yang diberikan SPV/Lead Ads BERSAMAAN dengan penetapan PIC: `target_spend` (ads spent), `target_gmv`, `target_roas`, `target_view` (impresi), `target_ctr`, `target_cvr` (+ `catatan`). Semua opsional, baris tanpa satu pun target ditolak (`ck_abt_min_satu`). Tulis: **SPV/Lead Ads atau Direksi saja** (M8-OA-4 — Advertiser tak pernah menetapkan targetnya sendiri); baca = lingkup baca brief induk (`private.jwt_can_read_brief`). **Nol prefix baru** (kunci alami `brief_id`, preseden `interview_riset_awal`). Bukan pengganti `ad_campaigns.target_kpi`: itu satu KPI teks per KAMPANYE, ini kontrak kerja per BRIEF yang ada sebelum kampanye lahir. Saran pre-fill dari `briefs.quantity_target` + `strategy_plans.target_*` — **tak pernah tersimpan otomatis**. Riwayat perubahan = `audit_log` before→after (`ads_target_set`/`ads_target_update`), bukan tabel versi |
| Laporan Mingguan Ads | — (PK `brief_id, iso_year, iso_week`) | BRF | M8 | **Keputusan pemilik 2026-08-14.** `ads_weekly_reports` — laporan Advertiser (PIC) per brief per minggu ISO (batas WIB Sen–Min, kunci ISO disimpan eksplisit seperti `WRR`): `analisa` performa + `saran` perbaikan (keduanya wajib) + `kendala` opsional. **NOL angka disimpan** — realisasi mingguan diturunkan saat baca dari `metric_entries` kampanye brief itu, dengan definisi rasio identik agregat M6D (T-3). **Append-only** (`forbid_mutation` UPDATE+DELETE; koreksi = laporan minggu berikutnya). Bukan duplikat M6D RM-D6 (`wrr_catatan_divisi`): RM-D6 per KLIEN oleh LEAD divisi ke rekap AM, ini per BRIEF oleh Advertiser — bahan mentahnya. Minggu selesai tanpa laporan ditandai `terlambat` saat baca; nol event notifikasi baru. Migrasi `20260814100000_m8_target_brief_laporan_mingguan.sql` |
...
- **M8 (target brief Ads, 2026-08-14):** realisasi mingguan per Brief-as-task — Σ spend / Σ GMV / ROAS (Σgmv÷Σspend) / view (Σimpressions) / CTR (Σclicks÷Σimpressions) / CVR (Σconversions÷Σclicks), di-bucket per minggu ISO WIB atas `metric_entries.period_end`; rasio terhadap target = realisasi÷target×100. Penyebut nol ⇒ `—`. Tak satu pun disimpan di `ads_weekly_reports` (yang menyimpan hanya narasi).
