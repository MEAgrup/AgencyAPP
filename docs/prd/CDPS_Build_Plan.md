...
| 20 | `CDPS_Module20_PDT_Laporan_Klien.md` | **New — added 2026-09-22** (permukaan laporan klien PDT: renderer HTML `klien`/`internal`, revisi insight + publikasi, portal klien, penulis `clients.total_sales`/Health Score/baseline Ads untuk PDT Rule 24, plus bagian Tokopedia & TikTok Ads Manager). Ini **port M14 → PDT**, bukan fitur baru — lahir dari audit pemilik "report klien vs PDT, fitur serupa tapi double" (`docs/DECISIONS.md` Open `M14-VS-PDT-DUPLIKASI`). Rencana eksekusi: `docs/plan/PLAN_PORT_M14_KE_PDT.md`. **Status 2026-09-22 malam: Gelombang A/B/C/D/E selesai** (PR #494/#496/#497; audit R11/R11.3 #499; migrasi C-01 di live + D-00 #501; Gelombang D #502 — `client-portal.ts` GANTI SUMBER ke tabel PDT, tes R11.3, tes penjaga R11 tetap hijau; Gelombang E #503 + PR E-03 lanjutan — PDT satu-satunya penulis `clients.total_sales` (R7) dan `metric_entries`/ROAS Attainment (E-03), kedua penulis M14 dimatikan lewat flag, `docs/DECISIONS.md` `M20-R6-HEALTH-SCORE-SNAPSHOT-LAMA`/`M20-E03-ADS-METRIC-ENTRIES`). Gelombang F (Tokopedia + TikTok Ads Manager, R8/R9): **F-01/F-02/F-03 selesai — `M20-TTAM-SAMPLE` ditutup, nol blocker tersisa di Gelombang F.** F-02 (#505 — kolom `pdt_fact_ads.tujuan`, guardrail R9 ditegakkan di query, migrasi live di produksi, `docs/DECISIONS.md` `M20-F02-PDT-FACT-ADS-TUJUAN`). F-01 (2026-09-23 — modul parser `tt_shop_analytics_tokopedia`, `pdt_fact_shop_daily.kanal`, bagian laporan "Toko Tokopedia"; sample asli pemilik (Ultrasleep) diverifikasi, `PDT_PARSER_VERSI` 4→5, `docs/DECISIONS.md` `M20-R8-F-01-TOKOPEDIA`). F-03 (2026-09-23, SELESAI PENUH — KEEMPAT modul TTAM: `tt_ads_manager_videoviews`/`consideration`/`follows`/`showcase`; pemilik mengunggah sample asli keempat tipe sekaligus menyusul PR videoviews-only pertama, membuktikan `videoviews` butuh `anyOf` TIGA varian EN/EN/ID sekaligus — signature DIKOREKSI kedua kalinya; dua kuirk baru ditemukan+ditutup di keempatnya (baris "Total of N results" sintetis difilter, `Ad name` tidak unik per baris digabung-jumlah mencegah pelanggaran `uq_pdt_fact_ads`), `PDT_PARSER_VERSI` 6→7, `docs/DECISIONS.md` `M20-R9-F-03-EMPAT-TIPE`). **F-04/F-05 SELESAI 2026-09-23** — kolom `pdt_fact_ads.hasil` (migrasi `20261207010000`, `PDT_PARSER_VERSI` 7→8) mengisi `vv_*`/`fol_*`/`sc_atc`/`sc_cost_atc` + funnel puncak langkah `atc` dari KEEMPAT modul TTAM; sekalian menutup bug F-03 (`sc_impresi`/`sc_klik` DULU salah sumber GMV Max, dikoreksi ke `tt_ads_manager_showcase`); `bangunLaporanKelengkapan` tidak lagi menuduh "Ads Manager belum punya modul" untuk rung yang sudah terisi. **Gelombang F SELESAI PENUH** — `docs/DECISIONS.md` `M20-R9-F-04-TAHAP-FUNNEL`. Satu item terbuka SENGAJA di luar scope: rung funnel `impresi` ("Impresi produk toko") tetap `null` permanen, gap `pdt_fact_shop_daily` terpisah tidak terkait Ads Manager. **Gelombang G (pencabutan M14) SELESAI 2026-09-23** — rute/FE M14 dicabut (`ReportPanel.tsx`/`ShopeeReportForm.tsx`/`InsightEditor.tsx`/`lib/report.ts`, 10 rute backend, blok `wire.ts` terkait), `client_reports`/`packages/core/src/report/**`/`packages/domain/src/report.ts` sengaja dipertahankan (riwayat + pemanggil hidup di luar M14), kontrol `tahap_fokus` dipindah ke tabel Platform sebelum `ReportPanel.tsx` dihapus, `docs/DECISIONS.md` `M20-R9-GELOMBANG-G-SELESAI`. **Dengan ini seluruh plan M20 (Gelombang A→G) TUNTAS.** |
| PX | `CDPS_ProductExchange_M1_M2.md` + `CDPS_ProductExchange_M3.md` | **New — added 2026-09-12, M3 ditambah 2026-09-15** (Product Exchange; bukan modul bernomor. PX-M2a mendarat 2026-09-12. **PX-M3-B mendarat 2026-09-15** — `px_sku_volume`/`px_sku_kategori`/`px_sku_eligibility`/`px_coverage_snapshot`/`px_coverage_push`, gerbang empat lapis, bridge coverage MCN, halaman Kandidat/Katalog PX — lihat `docs/DECISIONS.md` 2026-09-15 untuk deviasi PX-M3-01..08. **Aktivasi tick/hook DITUNDA** sampai PDT G1 `verified` di ≥10 klien, `docs/backlog/PX_M3_BACKLOG.md`. M3-A — sisi `mcnapp` — sesi terpisah, belum dikerjakan). |
...
- **Riset Awal Baseline Engine + M6B Route Surface + Brief inherit (RAB-01…RAB-20, `docs/backlog/RISET_AWAL_BASELINE_BACKLOG.md`)** — sits inside M6 / this wave (no wave shifted). Three clusters: **(a) Riset Awal baseline** — 4 baseline tables (`riset_awal_analisa`, `riset_awal_sumber_berkas`, `interview_riset_awal_isian`, `riset_awal_benchmark`), TikTok-Shop export analysis engine (registry adapter so a Shopee engine = plug an adapter), per-figure usulan→konfirmasi, Skor Kondisi Toko, prerequisite gate (`assertRisetAwalGate` blocks starting an interview until active-platform baselines exist); baseline then flows into Strategi Section B (`getBaselinePrefill`) — this is the pipeline that finally *satisfies* M6A Rule 5. **(b) M6B route surface** — `plan_row` write path (`createPlanRow`) + 11 M6B routes wired to `apps/api`, `route-parity` `KNOWN_GAPS` kept empty. **(c) Brief inherit (RAB-16/17)** — "satu klik, warisi semua": an `Aktif` Plan period inherits one Brief per qualifying `plan_row` to its PIC division (`inheritBriefsFromPlan`), AM fills only due date + priority; the manual STR- path stays served. **Corrects five PRD points (RAB-19) — see M6A D5/D18/OA-9/Rule 5 + M6B_Plan P3, `docs/DECISIONS.md` 2026-08-18.** New module PRD written: `docs/prd/CDPS_Module6_Interview.md` (RAB-18, the drift root).
- **M6D Rekap Hasil Mingguan (Weekly Result Update)** — built **at the end of this wave, after M7/M8/M9/M10 expose their metrics** (same dependency shape as M6B P-E auto metrics). Auto-generated weekly per active client; consolidates per-division production (Creative # video, KOL # creator, Live Stream # live) and movement metrics (total view, GMV, CTR, CVR, ROAS) from the owning modules, manual fallback where the system owns nothing. Rolls up into the monthly M6B P-E realisasi — never replaces it (single-source GMV guardrail). Covers **all active clients incl. Direct/`Tanpa Plan`**, closing the gap that non-plan services have no periodic results record. State machine #18; catalog v3 (+3 events). **Feeds the Client Health Report view (M13 §8 amendment) as its progress/results/complaint summary — view only, the Health Score's 7 components and weights are untouched.** Owner request, `docs/DECISIONS.md` 2026-08-12.
...
**Exit criteria:** Alpha-Digital-style client runs a full delivery cycle: Service → Briefs across ≥2 divisions → Tasks with live Speed Scores → one revision loop → one blocked interval excluded from turnaround → live-stream session reconciled. **Riset-awal path proven:** a client's Riset Awal is completed (active-platform baselines entered, each figure AM-confirmed, Skor Kondisi Toko computed) → the interview cannot start until that gate passes → the baseline flows into Strategi Section B → an `Aktif` Plan period inherits Briefs one-click to PIC divisions, with the manual STR- path still available.
