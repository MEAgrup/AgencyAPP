...
14. **Rule 1a — alternate door: creating a Strategi IS a `Butuh Plan` answer (⟟ 2026-08-27 DECISIONS, pemilik).** Numbered 14 in this list (added after Rule 13, not spliced in as literal item 1a) so "Rule 11"/"Rule 12" and every other cross-reference by number elsewhere in this codebase keep pointing at what they always pointed at — "Rule 1a" is this rule's name, not its position. For `Plan Ditentukan AM`, the G-B form is not the only way to answer. If the AM opens the Strategi (`STRG-`, Module 6A) create door directly for a service that has never been answered, submitting it **is** the `Butuh Plan` decision — recorded as an M6-OA-1 override (same column, same audit action as a manual override), not by fabricating a `service_plan_gate` row with guessed G-A attributes nobody actually answered. The G-B form stays available afterward for an AM who wants the full recommendation/trigger/reason trail on record; it is no longer a precondition for this tier once a Strategi already proves the decision. This alternate door does **not** apply to a service that is conclusively no-Plan — `Tanpa Plan` locked by the catalog with no escalation, or already decided `Tanpa Plan` in G-B — both still have no Strategi create door (Rule 11's escalation form is the only route for the former).
