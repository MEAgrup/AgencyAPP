...
3. **Scouted / Sales-registered leads are exclusive** — exactly one attempt, owner = the finder; not contestable (reference rule preserved).
   > ⚠️ **DEVIASI DIKETOK 2026-09-08 (FS-1/FS-2, feedback tim Sales) — lihat `docs/DECISIONS.md`.** Aturan asli di atas DIPERTAHANKAN sebagai catatan sejarah, tetapi TIDAK lagi berlaku apa adanya: prospek yang sudah didaftarkan sales A lalu menghubungi sales B kini menjadi **prospek bersama** — attempt kedua lahir lewat outcome `join`, tanpa persetujuan, pemilik lama diberi notifikasi, dan kepemilikan/komisi dibagi di antara keduanya lewat alokasi Closing. Saat salah satu menutup deal, rekannya ditutup `[Closed - Prospek Bersama]` (bukan `[Closed - Kalah Kompetisi]`, yang tetap berlaku untuk kontes lead `[Pool]` di Rule 1-2 dan Rule 5).
