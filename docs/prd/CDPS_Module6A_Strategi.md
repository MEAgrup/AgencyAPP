...
| D8 | Targets | Contract target = **floor**; AM sets a **stretch target** above it. ⟳ 2026-09-08 (O76): the floor is anchored to the Sales-agreed target GMV, with a ±20% tolerance band and Head of Account sign-off beyond it (Rule 7) |
...
| D14 | Sanggahan Target | **Advisory only** — never changes the contract floor. ⟳ 2026-09-08 (O76): it now argues against a floor anchored to the Sales-agreed `clients.target_gmv`, not against a figure the AM typed themselves — which is what gives the advisory something to argue with |
...
7. **⟳ 2026-09-08 (DECISIONS, O76): Target floor is the AM's number, but it is MEASURED against the agreement — and beyond ±20% it needs Head of Account's sign-off.** The floor is typed in Strategi and anchored to `clients.target_gmv`, the GMV target Sales agreed with the client on the Qualified form (`NOT NULL`, carried into `clients` at closing) and snapshotted onto the Strategi as `client_target_gmv`. Σ floor per contract month is compared to that anchor: within ±20% it saves freely; beyond it, a written reason is mandatory (`[penyesuaian target GMV di luar toleransi 20% wajib disertai alasan]`) and the row sits `menunggu_persetujuan` until the Head of Account approves the Strategi, which stamps it `disetujui`. After approval the floor is frozen by `trg_strategi_target_guard_floor` — read-only in the enforceable sense. The stretch target must still be `>=` floor; if the AM believes the floor is unachievable, they raise `Sanggahan Target` (Section D-7) which routes to SPV — it does not lower the floor. On renewal (R-03) the anchor is re-agreed: the renewal form carries a "target GMV periode ini" field defaulting to the previous number, and executing the renewal moves it onto `clients.target_gmv` (audited). **Superseded original:** "Target floor is read-only. The contract target is pulled from the Contract record (Module 4) and cannot be edited in Strategi." — there was never a contractual GMV figure to pull: `contracts` has no GMV column and the service catalogue carries no GMV, so for a year the floor was whatever the AM typed, and D-7 amounted to the AM disputing their own number (O57 item (b) → O76).
