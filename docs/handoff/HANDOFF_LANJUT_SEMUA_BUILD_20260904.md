...
| **KS-4 / KS-4b** 🟡 | ⚠️ **KOREKSI 2026-09-05 — separuh baris ini sudah basi.** Klaim *"closing ratio belum dihitung `salesperf.ts`"* **TIDAK BENAR**: `closing_ratio_qualified_pct` sudah ada sebagai `MetricKey` (`packages/domain/src/salesperf.ts:215`) dan dikunci CHECK DB `ck_sales_targets_metric_key`. `role_type` **`Sales`** juga sudah terdaftar di M14 sejak PR #295 (KS-4b), dengan bobot **0**. Yang benar-benar masih kosong hanyalah **daftar komponen + bobot skor Sales di M14 (Σ=100)** — itu keputusan pemilik, terdaftar sebagai **KS-4** di §6 handoff SESI 6. | Bobot M14 Sales (bukan lagi perhitungan rasionya) |
