...
7. **`commission_rule` seluruh 32 baris = interim `"0% of standard price"` (komisi Rp0).** Sales Head belum menetapkan aturan komisi per layanan untuk rate card ini. Nilai interim ini **WAJIB diisi Sales Head sebelum UAT komisi Wave 1** — tanpa ini, semua perhitungan komisi di modul M0/M5 untuk 32 layanan ini akan menghasilkan Rp0.
   **Klarifikasi istilah (2026-07-16, pertanyaan Nerissa):** yang dimaksud "komisi" di kolom ini adalah **komisi SALES** — insentif untuk salesperson yang menjual layanan tersebut (dibayar setelah pembayaran klien diverifikasi Finance, dibagi sesuai Sales Allocation) — **bukan** komisi/fee yang diterima agency dari klien. Pendapatan agency = harga layanan itu sendiri. Format isian: `"<N>% of standard price"` (dihitung dari subtotal baris kalkulator) atau `"flat Rp <N>"` per layanan.
   **⛔ UPDATE 2026-07-17 — TIDAK PERLU DIISI (anomali ini DITUTUP).** Keputusan Nerissa (DECISIONS O26/O24): komisi sales berjenjang mengikuti pencapaian dan dihitung **di luar CDPS**; sistem cukup menyediakan total pencapaian sales. `commission_rule` dibiarkan `0% of standard price` untuk seluruh 32 layanan (angka komisi di sistem tampil Rp0 — disengaja). Sales Head tidak perlu mengisi kolom ini; butir #2 di §5 instruksi pengisian menjadi tidak berlaku.
