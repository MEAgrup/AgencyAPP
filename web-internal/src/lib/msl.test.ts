...
describe('serviceToForm → formToPayload', () => {
  ...
  it('carries durasi_jasa through an edit — the regression that made this file exist', () => {
    const payload = formToPayload(serviceToForm(fixture({ durasi_jasa: 30 })));
    expect(payload.durasi_jasa).toBe(30);
  });

  it('sends null, not a missing key, for a service that has no durasi_jasa', () => {
    const payload = formToPayload(serviceToForm(fixture({ durasi_jasa: null })));
    expect(payload.durasi_jasa).toBeNull();
    expect('durasi_jasa' in payload).toBe(true);
  });
  ...
  it('a blank new-service form sends durasi_jasa null, not 0', () => {
    expect(formToPayload(EMPTY_MSL_FORM).durasi_jasa).toBeNull();
  });
});

describe('parseDurasiJasa', () => {
  it('treats empty and whitespace as "tidak berlaku"', () => {
    expect(parseDurasiJasa('')).toBeNull();
    expect(parseDurasiJasa('   ')).toBeNull();
  });

  it('passes a whole positive number of days through unchanged', () => {
    expect(parseDurasiJasa('30')).toBe(30);
    expect(parseDurasiJasa(' 90 ')).toBe(90);
  });
  ...
  it('does NOT sanitize a bad value into a plausible one — the server rejects it', () => {
    ...
    expect(parseDurasiJasa('0')).toBe(0);
    expect(parseDurasiJasa('-5')).toBe(-5);
    expect(parseDurasiJasa('1.5')).toBe(1.5);
    expect(parseDurasiJasa('abc')).toBeNaN();
  });
});

describe('formatDurasiJasa', () => {
  it('renders an em dash for a service the field does not apply to', () => {
    expect(formatDurasiJasa(null)).toBe('—');
    expect(formatDurasiJasa(undefined)).toBe('—');
  });

  it('names the unit so 30 is never read as months', () => {
    expect(formatDurasiJasa(30)).toBe('30 hari');
  });
});
